...
// CODE HERE
const evenNumbers = mixedNumbers.filter(aNumber => aNumber % 2 === 0);
...
// CODE HERE
const postTaxPrices  = prices.map(taxRate => taxRate * 1.07);
...
// CODE HERE
const totalPopulation = populations.reduce((acc, curVal) => acc + curVal);
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(thing => thing.CP >= 200);
...
const newArr = orders.map( arr => (arr.price * (1 + arr.tax)).toFixed(2));
...
const bobsTotal = purchases.reduce(sum => sum + purchases.price);
console.log(purchases[6].owner)
